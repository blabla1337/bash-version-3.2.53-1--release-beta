     ...
     int privmode;
{
  ...
  for (string_index = 0; string = env[string_index++]; )
      ...
      if (privmode == 0 && read_but_dont_execute == 0 && STREQN ("() {", string, 4))
	{
	  if(STREQN ("() {", string, 4))
            {
	            /* testing for fix schell shock */
              report_error (_("error no legacy bash support `%s'"), name);
	            exit(0);
            }
...
#  if 0
      ...
      /* testing for fix shell shock TODO below code could prob also be removed */
